...
struct coord {
    ...
    CF coord<ull> unsign() const {
        ...
        for(int i=0;i<dim;i++) {
            crd[i]=(ull)llabs(llrint((double)d[i]));
        }
        return crd;
    }
...
struct field {
    ...
    CF field(coord<long long int> bl, coord<long long int> tr, H val=1, unsigned short m=1): multiplier(m) {
        bl*=multiplier;
        tr*=multiplier;
    ...
    CF field(const coord<long long int> sz,H val=1, unsigned short m=1):field(sz*-1, sz, val, m){}
    template<typename T>CF field(const coord<T> sz, H val=1, unsigned short m=1):field(sz.round(), val, m){}
    template<typename T>CF field(const coord<T> bl, const coord<T> tr, H val=1, unsigned short m=1):field(bl.round(), tr.round(), val, m){}
#ifdef __NVCC__
    field<H>* push() {
        field<H> *dest;
        auto cpy=*this;
    ...
    template<typename T>CF ull coordZuID(coord<T> x) {
        ull id=0;
        x*=(T)multiplier;
        auto cvrtCrd=x.round()-(*baseCrd);
        for(di i=0; i<x.dim; i++) {
            ull inRes=(ull)llrint((double)cvrtCrd[i]);
            di j=i;
            while(j!=0) {
                j--;
                inRes*=sL[j];
...
struct aitemu {
    aitemu *mae=nullptr, *tuki=nullptr;
    coord<double> val;
    CF aitemu(coord<double> x, aitemu *a=nullptr, aitemu *b=nullptr): val(x), mae(a), tuki(b){}
    CF ~aitemu() {
        val.~coord();
        delete mae;
        delete tuki;
    ...
    CF aitemu* before(int count) { 
        if(count==0) return this;
        return (*mae).before(--count);
    }
//    aitemu& operator=(const aitemu &other) {
//        //printf("move ro3");
//        if (this == &other) return *this;
//    }
    CF void insertAfter(aitemu* maePtr) {
        //if(mae!=nullptr) throw "Pointer Before!";
#ifdef __CUDA_ARCH__
        __syncthreads();
#endif
        mae=maePtr;
    ...
    CF auto append(coord<double> x) {
        //if(this->tuki!=nullptr) throw "Something after this, can't append!";
        aitemu *tmp=new aitemu(x);
        tmp->insertAfter(this);
        return tmp;
    }
    CF auto prepend(coord<double> x) {
        //if(this->mae!=nullptr) throw "Something before this, can't prepend!";
        aitemu *tmp=new aitemu(x);
        tmp->insertBefore(this);
        return tmp;
    }
    CF void insert(aitemu* ptr1, aitemu* ptr2) {
        if((*ptr1).mae==ptr2) return (*this).insertBefore(ptr1);
        return (*this).insertBefore(ptr2);
    }
#ifdef __NVCC__
    aitemu* push() {
        aitemu* dest;
        cudaMalloc((void **)&dest, sizeof(*this));
        double *dd=this->val.d;
        cudaMalloc((void **)&dd, sizeof(double)*this->val.dim);
        cudaMemcpy(dd, this->val.d, sizeof(double)*this->val.dim, cudaMemcpyHostToDevice);
        auto oldDD=this->val.d;
        this->val.d=dd;
...
#endif
    struct iterator {
        friend auto operator*(const iterator& x) { return *(x.it); }
        friend auto operator++(iterator& x) { return (*(x.it)).tuki; }
        friend auto operator++(iterator& x, int) { iterator tmp=x; ++(x); return tmp; }
        friend auto operator--(iterator& x) { return (*(x.it)).mae; }
        friend auto operator--(iterator& x, int) { iterator tmp=x; --x; return tmp; }
        //friend auto operator->() return it;
        friend bool operator==(const iterator& a, const iterator& b) { return a.it==b.it; }
        friend bool operator!=(const iterator& a, const iterator& b) { return a.it!=b.it; }
        //using difference_type = coord<double>;
        using value_type = aitemu;
        using iterator_concept = std::bidirectional_iterator_tag;
        iterator(aitemu* x): it(x){}
        private:
            aitemu* it;
    };
    auto begin() const { return iterator(this->tuki); }
};
...
//    }
    CF void insert(aitemu* ptr1, aitemu* ptr2) {
        if((*ptr1).mae==ptr2) return (*this).insertAfter(ptr2);
